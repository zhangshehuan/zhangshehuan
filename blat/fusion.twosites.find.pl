...
#*****************************************************************************
# FileName: fusion.twosites.find.pl
			...
			if ($matchlen{$big}>=$max_len) {
				...
				foreach my $item (@sort) {
					...
					if ($flag==0 and $item ne $big) {
						...
						if ($mark==2) {
							my $left=int(302-$Sbjct_beg);
							my $right=int($Sbjct_end-301);
							if ($left<$right) {
								$pick{$item}.="\t$left";
							}else {
								$pick{$item}.="\t$right";
...
my @filt=keys %filter;
my $f=@filt;
print OUT "all=$a, filter=$f, single=$s, multiple=$m\n";
close OUT;
